...
//----Terminan funciones Drag--------//
      ...
      function preguntaArrastrar() {
        ...
        strVar += "          placeholder=\"Instrucciones para el alumno\" value=\"Arrastra las palabras a la casilla correcta\"> <\/div>";
...
function validarPreguntaDrag(){
  ...
  for(i=0;i<cuestionarios.length;i++){
      ...
      var contenedorErrores=cuestionario.querySelectorAll("div.contenedorErrores")[0];
      var errorHTML=cuestionario.querySelectorAll("div.Error")[0];
      var errorOL=cuestionario.querySelectorAll("div.ErrorOL")[0];
      //// ////Inserción de mensajes por elementos no posicionados o inexistentes///////

      //Si existen elementos sin definir, o reactivos sin respuestas...
       if((errorEX || errorCont )&& !errorOL){
        var strVar="";
            strVar += "      <div class=\" row justify-content-center mb-4\">";
            strVar += "          <div class=\"ErrorOL fw-bold col-10 mx-auto p-3 m-1 text-center mb-3 border border-danger\">";
            strVar += "            <p class=\"text-danger fs-5 at-2 py-3 mensaje\" style=\"line-height: 5px;\">Error: "+mensajeEV+"<\/p>";
            strVar += "          <\/div>";
            strVar += "        <\/div>";
        ...
        if(typeof(errorOL)!="undefined") {
          var msjOL=errorOL.querySelectorAll("p.mensaje")[0];
        //Si aún existe el mensaje de error , actualizar el mensaje de que falta(instrucción o imagen)
        if(msjOL){
          msjOL.textContent="Error: "+mensajeEV;
        }
        //Si hay mensaje de erorr y reactivos sin contenido
        if(msjOL&&errorCont){
          msjOL.textContent+="Hay Reactivos sin respuesta";
        }
        //Si el errorHTML existe y ya no falta ni la imagen ni las instrucciones
        if(!errorEX && !errorCont){
          errorOL.remove();
        }
      }
...
function crearQuizz(){
  ...
  validarPreguntaDrag();
    ...
    var erroresObjetoLienzo=document.getElementsByClassName("ErrorOL")[0];
    if(errores ||erroresRespuesta || erroresImagen||erroresObjetoLienzo||validacion.error){
